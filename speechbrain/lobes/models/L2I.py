...
class PsiOptimized(nn.Module):
    ...
    def __init__(
        ...
        if use_adapter:
            ...
            # If no adapter used, force adapter_reduce_dim to False to avoid errors
            self.adapter_reduce_dim = False
    ...
    def forward(self, hs):
        ...
        # Force output to have time dimension T
        if out.shape[2] != self.T:
             out = F.interpolate(out, size=(self.T, out.shape[3]), mode='bilinear', align_corners=False)
