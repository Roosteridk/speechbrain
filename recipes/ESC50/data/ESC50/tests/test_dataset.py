...
def test_previews(meta):
    ...
    recordings = (
        meta.groupby("target")["filename"]
        .apply(lambda cat: cat.sample(1))
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        for index in range(len(recordings)):
            ...
            spec = librosa.feature.melspectrogram(
                signal, sr=44100, n_fft=2205, hop_length=441
            )
            ...
            ax.imshow(
                spec, origin="lower", interpolation=None, cmap="viridis", aspect=1.1
            )
        ...
        subprocess.call(
            ["convert", "-delay", "100", "-loop", "0", f"{tmpdir}/*.png", "_esc50.gif"]
        )
